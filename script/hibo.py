...
force_year_path = "data/hindi/forceyear_merged.json"
if os.path.exists(force_year_path):
    try:
        with open(force_year_path, "r", encoding="utf-8") as f:
            for row in json.load(f):
                slug = row.get("s")
                fy = row.get("fy")
                if slug and fy:
                    FORCE_YEAR_MAP[slug] = int(fy)
        print("Force year entries:", len(FORCE_YEAR_MAP))
    except Exception as e:
        print("Failed loading forceyear:", e)

# ---------- NEW: Load corrected dates ----------
CORRECTED_DATES = {}
corrected_dates_path = "data/hindi/correcteddates.json"
if os.path.exists(corrected_dates_path):
    try:
        with open(corrected_dates_path, "r", encoding="utf-8") as f:
            CORRECTED_DATES = json.load(f)
        print("Corrected dates entries:", len(CORRECTED_DATES))
    except Exception as e:
        print("Failed loading correcteddates:", e)
# ------------------------------------------------
...
def round05(v):
    if v < 0.5:
        return round(v, 2)
    return float(
        (Decimal(str(v)) * 20).quantize(Decimal("1"), rounding=ROUND_HALF_UP) / Decimal("20")
    )
...
movie_titles = {}
movies = defaultdict(lambda: defaultdict(lambda: {"gross": 0, "sold": 0, "shows": 0, "seats": 0}))
...
for canonical_name, days_data in movies.items():
    ...
    if slug in CORRECTED_DATES:
        corr = CORRECTED_DATES[slug]
        if "rd" in corr:
            try:
                # parse DD-MM-YYYY
                corr_rd = datetime.strptime(corr["rd"], "%d-%m-%Y").strftime("%Y-%m-%d")
                release_date = corr_rd
                rd = datetime.strptime(release_date, "%Y-%m-%d")
                print(f"Corrected date for {slug}: {release_date}")
            except Exception as e:
                print(f"Error parsing corrected date for {slug}: {e}")
        if "premiere" in corr:
            has_premieres = corr["premiere"]
            print(f"Corrected premiere for {slug}: {has_premieres}")
    # ------------------------------------------

    release_year = FORCE_YEAR_MAP.get(slug, int(release_date[:4]))
    if slug in FORCE_YEAR_MAP:
        print(f"FORCED YEAR: {slug} -> {release_year}")
    ...
    if os.path.exists(output_path):
        with open(output_path, "r", encoding="utf-8") as f:
            old_json = f.read()
        if old_json == new_json and not (FORCE_REBUILD and release_year == REBUILD_YEAR):
...
# ---- Build index files for each year ----
for idx_year in range(2023, YEAR + 1):
    folder = f"{idx_year}/hindi"
    ...
    for filename in os.listdir(folder):
        ...
        max_day = 0
        for day in movie.get("days", []):
            max_day = max(max_day, day.get("d", 0))
        ...
        movies_list.append({
            ...
            "n": movie.get("tn", 0),
            "d": max_day,
            "d1os": movie.get("d1os", 0),
            "d2os": movie.get("d2os", 0),
            "d3os": movie.get("d3os", 0),
            "d4os": movie.get("d4os", 0),
            "wos": movie.get("wos", 0),
            "tos": movie.get("tos", 0),
            "vd": movie.get("vd", ""),
        })

    movies_list.sort(key=lambda x: x["n"], reverse=True)
    ...
    new_index_json = json.dumps(index_payload, ensure_ascii=False, separators=(",", ":"))
